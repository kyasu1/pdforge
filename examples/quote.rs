...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let pdforge = pdforge::PDForgeBuilder::new("QUOTE_EXAMPLE".to_string())
    ...
    let sample_items = vec![
        ...
        (
            "腕時計 カルティエ タンクフランセーズ W343234 #98723433C2343 箱付　保付 18KYG 美品",
            "320,000",
            "260,000",
        ),
        ...
        (
            "エルメス バーキン 35cm シェーブル □U刻印 箱付き 袋付き",
            "1,200,000",
            "1,000,000",
        ),
    ...
    let bytes: Vec<u8> = pdforge.render("quote", vec![inputs], Some(table_data), None)?;
