...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Starting memory-efficient large table PDF generation...");

    let setup_start = Instant::now();

    // Create PDForge instance
    let pdforge = pdforge::PDForgeBuilder::new("Memory Efficient Table Test".to_string())
    ...
    // Generate large dataset - 2500 rows (approx 100 pages)
    let total_rows = 2500;
    let mut rows = Vec::with_capacity(total_rows);

    println!("Generating {} rows of data...", total_rows);
    ...
    let output_path = "./examples/pdf/memory-efficient-table.pdf";
